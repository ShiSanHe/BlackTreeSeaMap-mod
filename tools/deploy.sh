#!/usr/bin/env bash
# 黑流树海 Mod 编译 + 打包脚本（guzhenren 模式：默认只打包到本地 dist/，由用户手动部署到游戏 mods/）
# 用法:
#   tools/deploy.sh [Configuration]            # 打包到 dist/（默认 Debug）
#   INSTALL_DIR=<游戏mods目录> tools/deploy.sh  # 附加：显式安装到指定目录
set -euo pipefail
...
# RitsuLib 运行时依赖：mod_manifest 声明依赖它，一并打包让玩家一次复制到位
...
echo "    GAME_MODS=\"<Slay the Spire 2>/SlayTheSpire2.app/Contents/MacOS/mods\""
